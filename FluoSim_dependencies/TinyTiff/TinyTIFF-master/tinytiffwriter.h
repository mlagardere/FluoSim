...
/** \brief maximum size of the imageDescription field in the first frame (including trailing \c 0, which has to be present!)
  * \ingroup tinytiffwriter
  */
TINYTIFF_DLL_SPEC int TinyTIFFWriter_getMaxDescriptionTextSize();
...
/*! \brief create a new TIFF file
  ...
  */
TINYTIFF_DLL_SPEC TinyTIFFFile* TinyTIFFWriter_open(const char* filename, uint16_t bitsPerSample, uint32_t width, uint32_t height);
...
/*! \brief write a new image to the give TIFF file
  ...
  */
TINYTIFF_DLL_SPEC void TinyTIFFWriter_writeImage(TinyTIFFFile* tiff, void* data);
TINYTIFF_DLL_SPEC void TinyTIFFWriter_writeImage(TinyTIFFFile* tiff, float* data);
TINYTIFF_DLL_SPEC void TinyTIFFWriter_writeImage(TinyTIFFFile* tiff, double* data);
 ...
 */
TINYTIFF_DLL_SPEC void TinyTIFFWriter_close(TinyTIFFFile* tiff, double pixel_width=0, double pixel_height=0, double frametime=0, double deltaz=0);
...
/*! \brief close a given TIFF file and write the given string into the IMageDescription tag of the first frame in the file.
    ...
    This function also releases memory allocated in TinyTIFFWriter_open() in \a tiff.
 */
TINYTIFF_DLL_SPEC void TinyTIFFWriter_close(TinyTIFFFile* tiff, char* imageDescription);
