...
#if LIBRARY == TRUE
#	ifdef BUILDING_TINYTIFF_DLL
#		define TINYTIFF_DLL_SPEC __declspec(dllexport)
#	else
#		define TINYTIFF_DLL_SPEC __declspec(dllimport)
#endif
#else
#	define TINYTIFF_DLL_SPEC
#endif
...
/*! \brief open TIFF file for reading
  ...
  */
TINYTIFF_DLL_SPEC TinyTIFFReaderFile* TinyTIFFReader_open(const char* filename);
 ...
 */
TINYTIFF_DLL_SPEC void TinyTIFFReader_close(TinyTIFFReaderFile* tiff);
 ...
 */
TINYTIFF_DLL_SPEC const char* TinyTIFFReader_getLastError(TinyTIFFReaderFile* tiff);
 ...
 */
TINYTIFF_DLL_SPEC int TinyTIFFReader_wasError(TinyTIFFReaderFile* tiff);
 ...
 */
TINYTIFF_DLL_SPEC int TinyTIFFReader_success(TinyTIFFReaderFile* tiff);
 ...
 */
TINYTIFF_DLL_SPEC int TinyTIFFReader_hasNext(TinyTIFFReaderFile* tiff);
 ...
 */
TINYTIFF_DLL_SPEC int TinyTIFFReader_readNext(TinyTIFFReaderFile* tiff);
 ...
 */
TINYTIFF_DLL_SPEC uint32_t TinyTIFFReader_getWidth(TinyTIFFReaderFile* tiff);
 ...
 */
TINYTIFF_DLL_SPEC uint32_t TinyTIFFReader_getHeight(TinyTIFFReaderFile* tiff);

/*! \brief return true if the current frame is constituted of sub images, false if not.
 *	Introduced by M. Lag. 2019/02/22
	\ingroup TinyTIFFReader

	\param tiff TIFF file

 */
TINYTIFF_DLL_SPEC bool TinyTIFFReader_hasSubImages(TinyTIFFReaderFile* tiff);

/*! \brief return the number of sub images in the current frame.
 *  Return 0 if the current frame does not pocess sub Images.
 *	Introduced by M. Lag. 2019/02/22
	\ingroup TinyTIFFReader
 ...
 */
TINYTIFF_DLL_SPEC uint32_t TinyTIFFReader_getNbSubImages(TinyTIFFReaderFile* tiff);
 ...
 */
TINYTIFF_DLL_SPEC std::string TinyTIFFReader_getImageDescription(TinyTIFFReaderFile* tiff);
 ...
 */
TINYTIFF_DLL_SPEC uint16_t TinyTIFFReader_getSampleFormat(TinyTIFFReaderFile* tiff);
 ...
 */
TINYTIFF_DLL_SPEC uint16_t TinyTIFFReader_getBitsPerSample(TinyTIFFReaderFile* tiff, int sample=0);
 ...
 */
TINYTIFF_DLL_SPEC uint16_t TinyTIFFReader_getSamplesPerPixel(TinyTIFFReaderFile* tiff);
 ...
 */
TINYTIFF_DLL_SPEC int TinyTIFFReader_getSampleData(TinyTIFFReaderFile* tiff, void* buffer, uint16_t sample);


/*! \brief read the given sample from the current specified frame sub image into the given buffer,
		   the byteorder is transformed to the byteorder of the system!
*	Introduced by M. Lag. 2019/02/22
	\ingroup TinyTIFFReader
	...
	\param tiff TIFF file
	\param buffer the buffer this function writes into
	\param subImageIdx: index of the sub image to be read in the current frame.
	\param sample the sample to read [default: 0]
	\return \c TRUE (non-zero) on success
 ...
 */
TINYTIFF_DLL_SPEC int TinyTIFFReader_getSampleDataSubImage(TinyTIFFReaderFile* tiff, \
												void* buffer, uint16_t subImageIdx, uint16_t sample);
 ...
 */
TINYTIFF_DLL_SPEC uint32_t TinyTIFFReader_countFrames(TinyTIFFReaderFile* tiff);
