		...
		for ( i=0; i<ifd_count; i++) {
			...
			switch(ifd.tag) {
				...
				case UIC1Tag :
					break;
				case UIC2Tag :
					tiff->currentFrame.nbSubImages = ifd.count;
					break;
				case UIC3Tag : break;
				case UIC4Tag : break;
				default:
...
int TinyTIFFReader_getSampleDataSubImage(TinyTIFFReaderFile* tiff, void* buffer, uint16_t subImageIdx, uint16_t sample) {
	if (tiff) {
		if (tiff->currentFrame.compression!=TIFF_COMPRESSION_NONE) {
			tiff->wasError=TRUE;
			strcpy(tiff->lastError, "the compression of the file is not supported by this library\0");
		...
		if(tiff->currentFrame.nbSubImages <= 0) {
			tiff->wasError=TRUE;
			strcpy(tiff->lastError, "the current frame does not contain sub images\0");
			return FALSE;
		}
		if(subImageIdx >= tiff->currentFrame.nbSubImages) {
			tiff->wasError=TRUE;
			strcpy(tiff->lastError, "the current frame does not contain this sub image\0");
			return FALSE;
		}
		...
		uint32_t strip_ocunt = tiff->currentFrame.stripcount;
		uint32_t plane_offset = subImageIdx * (tiff->currentFrame.stripoffsets[strip_ocunt-1] +
							  tiff->currentFrame.stripbytecounts[strip_ocunt-1] - tiff->currentFrame.stripoffsets[0]);
		...
		if (tiff->currentFrame.stripcount>0 && tiff->currentFrame.stripbytecounts && tiff->currentFrame.stripoffsets) {
			...
			if (tiff->currentFrame.bitspersample[sample]==8) {
				for (s=0; s<tiff->currentFrame.stripcount; s++) {
					...
					uint8_t* tmp=(uint8_t*)calloc(tiff->currentFrame.stripbytecounts[s], sizeof(uint8_t));
					//fseek(tiff->file, tiff->currentFrame.stripoffsets[s], SEEK_SET);
					TinyTIFFReader_fseek_set(tiff, plane_offset + tiff->currentFrame.stripoffsets[s]);
					//fread(tmp, tiff->currentFrame.stripbytecounts[s], 1, tiff->file);
					TinyTIFFReader_fread(tmp, tiff->currentFrame.stripbytecounts[s], 1, tiff);
					uint32_t offset=s*tiff->currentFrame.rowsperstrip*tiff->currentFrame.width;
					//printf("          bufferoffset=%u\n", offset);
...
bool TinyTIFFReader_hasSubImages(TinyTIFFReaderFile* tiff) {
	if (tiff) {
		return tiff->currentFrame.nbSubImages > 0 ? true : false;
	}
	return false;
}

uint32_t TinyTIFFReader_getNbSubImages(TinyTIFFReaderFile* tiff) {
	if (tiff) {
		return tiff->currentFrame.nbSubImages;
	}
	return 0;
}
